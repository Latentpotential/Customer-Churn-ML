...
def validate_telco_data(df) -> Tuple[bool, List[str]]:
    ...
    # Coerce numeric columns before validation. The raw CSV stores TotalCharges as
    # strings (with ~11 blank entries), which breaks numeric range checks with a
    # "str vs int" TypeError. Coerce on a copy so the caller's df is untouched.
    df = df.copy()
